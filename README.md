...
- 图表窗口新增“里程碑”页：按每 1、5、10、50、100 亿 Tokens 查看各段耗时、当前进度与历史阶段；采用当前来源的全部可用历史，不受首页时间范围影响。
- “用量与额度趋势”窗口新增独立的“累计里程碑”子页，可直接在曲线上查看里程碑节点；点击节点高亮对应阶段并查看耗时。日期范围独立于主页和用量图表，累计值仍从现存完整历史开始。
...
- 自动识别日志中新出现的模型；首次出现时从感知差异明显的候选色中自动分配深浅主题配色，并在升级和重启后保持不变。
...
在线价格清单只用于 Standard 短上下文 API 等效估算，并非账单。软件读取本项目维护、按 OpenAI 官方价格页核对的受限 JSON 清单，不抓取价格网页；Fast、Batch、Flex、长上下文、地区及工具费用不包含在此估算中。用户自定义价格始终优先。

额度历史从启用此版本后开始记录，仅保存成功的在线查询，不回填旧日志。额度重置会显示为曲线跳变，软件未运行期间不会产生记录。

里程碑的“第 2 个 1 亿”指累计 1 亿到 2 亿这一段。第一个阶段从现存首条有效用量记录开始，其后从上一个节点开始，耗时包括离线和未使用时间。一次记录跨过多个节点时，无法拆分的耗时显示为未知；只有日期的历史汇总显示时间范围。补入、替换或移除旧记录后会重新计算，不代表账号完整历史。

设置、价格、模型配色、额度历史和缓存保存在当前 Windows 用户的 `%LOCALAPPDATA%\CodexUserData`。同一用户升级后继续读取此目录：退出旧版，再解压运行新版即可。该用户数据目录及旧版 `data` 文件夹属于个人数据，请勿随问题反馈上传。
