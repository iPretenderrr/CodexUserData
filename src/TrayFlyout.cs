...
namespace CodexUserData
{
    internal sealed class TrayFlyout : Window
    {
        ...
        {
            bool opening=!IsVisible;Dismissing=false;transition++;if(opening){Opacity=0;Show();PositionAt(anchor);}else PositionAt(anchor);
            double from=opening?0:Opacity;BeginAnimation(OpacityProperty,null);if(!SystemParameters.ClientAreaAnimation){Opacity=1;return;}
            var fade=new System.Windows.Media.Animation.DoubleAnimation(from,1,TimeSpan.FromMilliseconds(180)){EasingFunction=new System.Windows.Media.Animation.CubicEase{EasingMode=System.Windows.Media.Animation.EasingMode.EaseOut}};
            Opacity=1;BeginAnimation(OpacityProperty,fade);
        }
        internal void Dismiss()
        {
            int revision=++transition;if(!IsVisible)return;if(!SystemParameters.ClientAreaAnimation){Hide();return;}Dismissing=true;
            var fade=new System.Windows.Media.Animation.DoubleAnimation(Opacity,0,TimeSpan.FromMilliseconds(120));fade.Completed+=delegate{if(revision!=transition)return;Dismissing=false;Hide();BeginAnimation(OpacityProperty,null);Opacity=1;};BeginAnimation(OpacityProperty,fade);
        }
        internal TrayFlyout(Action show,Action charts,Action refresh)
        {
            ...
            FontFamily=new FontFamily("Segoe UI, Microsoft YaHei UI");Theme.InstallStyles(this);
            Closed+=delegate{transition++;Dismissing=false;BeginAnimation(OpacityProperty,null);};
            ...
            var header=new DockPanel{Margin=new Thickness(0,0,0,12)};layout.Children.Add(header);var close=Theme.ToolbarButton("expand","收起额度速览");close.Click+=delegate{Hide();};DockPanel.SetDock(close,Dock.Right);header.Children.Add(close);
            ...
            button("打开悬浮窗",()=>{Hide();show();});button("用量图表",()=>{Hide();charts();});button("刷新额度",refresh);
        }
        internal void Apply(IEnumerable<QuotaBucket> quotas,UsageSnapshot snapshot,string scope,string status)
        {
            body.Children.Clear();long now=LocalCodexUsage.Unix(DateTime.Now);var all=quotas.ToArray();var general=all.FirstOrDefault(q=>q.Id=="codex");
        ...
        {
            if(bucket==null||window==null||bucket.ObservedAt>now+5||now-bucket.ObservedAt>300||(window.ResetsAt>0&&window.ResetsAt<=now)||Double.IsNaN(window.UsedPercent)||Double.IsInfinity(window.UsedPercent))return null;
