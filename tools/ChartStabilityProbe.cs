...
namespace CodexUserData
{
    internal static class ChartStabilityProbe
    {
        ...
        internal static async Task Run(string root)
        {
            ...
            bool weeklyHeatMatches=true;for(int i=0;i<modeSource.Length;i++){DateTime date=DateTime.ParseExact(modeSource[i].Date,"yyyy-MM-dd",CultureInfo.InvariantCulture);DateTime sunday=date.AddDays(-(int)date.DayOfWeek);long expected=modeSource.Where(d=>{DateTime candidate=DateTime.ParseExact(d.Date,"yyyy-MM-dd",CultureInfo.InvariantCulture);return candidate>=sunday&&candidate<sunday.AddDays(7);}).Sum(d=>d.Tokens);weeklyHeatMatches&=weeklyHeat[i].Date==modeSource[i].Date&&weeklyHeat[i].Tokens==expected&&weeklyHeat[i].DisplayLabel.StartsWith("周 ");}
            StabilityProbe.Check(weeklyHeat.Length==modeSource.Length&&weeklyHeatMatches,"weekly heatmap preserves calendar cells and applies each Sunday-based week total only to that week's dates");
            StabilityProbe.Check(cumulativeHeat.Length==modeSource.Length&&cumulativeHeat.Last().Tokens==modeSource.Sum(d=>d.Tokens)&&cumulativeHeat.Last().DisplayLabel.StartsWith("累计至 "),"cumulative heatmap keeps calendar dates and grows from the visible 180-day window start");
            ...
            try
            {
                ...
                panel.SetHeatAggregation("weekly");heat.Choose(heat.Days.Length-1,true);StabilityProbe.Check(heat.Days.Length==180&&heat.WeekStartsSunday&&heat.Days.Last().DisplayLabel.StartsWith("周 ")&&heatDetail.Heading.StartsWith("周 ")&&AutomationProperties.GetItemStatus(heatModes["weekly"])=="已选中"&&AutomationProperties.GetItemStatus(trendModes["daily"])=="已选中","heatmap weekly mode uses Sunday rows and grouped detail without changing the trend mode");
                long fullHeatTotal=data.Daily.Sum(d=>d.Tokens);panel.SetHeatAggregation("cumulative");StabilityProbe.Check(heat.Days.Last().Tokens==fullHeatTotal&&!heat.WeekStartsSunday&&AutomationProperties.GetItemStatus(heatModes["cumulative"])=="已选中"&&AutomationProperties.GetItemStatus(trendModes["daily"])=="已选中","heatmap cumulative mode is independent and ends at the 180-day total");panel.SetHeatAggregation("daily");
