    ...
    internal static class ChartStabilityProbe
    {
        ...
        internal static async Task Run(string root)
        {
            ...
            try
            {
                ...
                StabilityProbe.Check(cacheTrend.IsRate&&cacheTrend.Days.Length==data.Daily.Length&&cacheTrend.WindowDays==14,"the token custom range leaves the cache hit range unchanged");
                ...
                panel.SetAggregation("weekly");StabilityProbe.Check(trend.Days.Length==1&&DateTime.ParseExact(trend.Days[0].Date,"yyyy-MM-dd",CultureInfo.InvariantCulture).DayOfWeek==DayOfWeek.Sunday&&trend.Days[0].Tokens==1000&&cacheTrend.WindowDays==14,"weekly token mode does not aggregate the independent cache hit curve");
                panel.SetAggregation("cumulative");StabilityProbe.Check(trend.Days.Length==customTimeline.Length&&trend.Days.Last().Tokens==1000&&cacheTrend.WindowDays==14,"cumulative token mode does not alter the independent cache hit curve");panel.SetAggregation("daily");
                panel.SetRange(7);await Task.Delay(25);window.UpdateLayout();
                panel.BeginCacheCustomRange(customFrom,customTo,"Generated chart fixture");panel.ApplyCacheCustomRange(customSnapshot,"Generated chart fixture",customFrom,customTo);await Task.Delay(20);
                StabilityProbe.Check(cacheTrend.Days.Length==customTimeline.Length&&cacheTrend.WindowDays==customTimeline.Length&&trend.WindowDays==7&&AutomationProperties.GetItemStatus(cacheCustomButton)=="已选中","cache custom timestamps are independent from the token preset range");panel.SetCacheRange(14);
                ...
                StabilityProbe.Check(cacheTrend.IsRate&&!cacheTrend.IsHourly&&cacheTrend.WindowDays==14,"the token today range leaves the cache hit preset unchanged");panel.SetCacheRange(1);
                StabilityProbe.Check(cacheTrend.IsRate&&cacheTrend.IsHourly&&cacheTrend.Days.Length==24&&cacheTrend.WindowDays==1,"the cache hit chart independently switches to today's hourly range");
