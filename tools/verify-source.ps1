param([string]$OutputDirectory=(Join-Path $PSScriptRoot '..\.build\source-verification'),[switch]$IslandOnly)
...
if($IslandOnly){& $binary $destination --island-only}else{& $binary $destination}
